...
public class TestLobApiTest extends TestDb {
    ...
    void testUnsupportedOperations() throws Exception {
        ...
        assertThrows(Exception.class,()-> clob.
                truncate(0));
        ...
        assertThrows(Exception.class,()-> blob.
    ...
    private void testLobStaysOpenUntilCommitted() throws Exception {
        ...
        assertEquals(0L, c1.length());
        assertEquals(0L, b1.length());
        assertEquals("", c1.getSubString(1, 0));
        assertEquals(0, b1.getBytes(1, 0).length);

        assertEquals((long)charsSmall.length, c2.length());
        assertEquals((long)bytesSmall.length, b2.length());
        assertEquals(dSmall, c2.getSubString(1, (int) c2.length()));
        assertArrayEquals(bytesSmall, b2.getBytes(1, (int) b2.length()));

        assertEquals((long)chars.length, c3.length());
        assertEquals((long)bytes.length, b3.length());
        assertEquals(d, c3.getSubString(1, (int) c3.length()));
        assertArrayEquals(bytes, b3.getBytes(1, (int) b3.length()));
        stat.execute("drop table test");
        conn.close();
    }
    ...
    private void testInputStreamThrowsException(final boolean ioException)
        ...
        assertArrayEquals(new String(new char[10000]).toCharArray(), str.toCharArray());
        assertArrayEquals(new byte[0], rs.getBytes(2));
        rs.next();
        assertEquals("", rs.getString(1));
        assertArrayEquals(new byte[10000], rs.getBytes(2));
        stat.execute("drop table test");
        conn.close();
    }
    ...
    private void testBlob(int length) throws Exception {
        ...
        assertEquals((long)length, b3.length());
    ...
    private void testClob(int length) throws Exception {
        ...
        rs.next();
        Clob c2 = rs.getClob(2);
        assertEquals((long)length, c2.length());
        String s = c.getSubString(1, length);
        @SuppressWarnings("UnusedAssignment") String s2 = c2.getSubString(1, length);
        while (rs.next()) {
            c2 = rs.getClob(2);
            assertEquals((long)length, c2.length());
